...
function randomJoke() {
    ...
    var random = Math.floor(Math.random() * 378)
    console.log(random);
    console.log(jokes[random]);
} //   if user ask random joke

/* 
1 - read all the jokes 
2 - check for particular type jokes - programming
3 - if match new arry to add all the particular jokes
*/
...
// programming
function filterProgrammingJokes() {
    let programmingJokes = []
    jokes.forEach(joke => {
        if (joke.type === "programming") {
            programmingJokes.push(joke)
        }
    });
...
function generalJokes() {
    ...
    jokes.forEach(joke => {
        if (joke.type === "general") {
            generalJokes.push(joke)
        }
    });
    var randomJoke = Math.floor(Math.random() * generalJokes.length)
    console.log(generalJokes[randomJoke]);
}

// print particular id joke 235

console.log(jokes[0].id);
var targetedJoke =[];
let targetedJokeId ="https://emaple.com/pid=123"|235
jokes.forEach(joke => {
    if (joke.id===targetedJokeId) 
    {
      targetedJoke.push(joke)
        
    }
});
console.log(targetedJoke);